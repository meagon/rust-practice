...
fn main() {
    ...
    match args.len() {
        0 => unreachable!(),
        2 => { 
           let mut aa =  make_and_print_request(args.get(1).as_slice());
...
fn make_and_print_request(url: &str) -> String {
    ...
    let mut response = match request.read_response() {
        ...
        Err(_request) => { fail!("This example can progress no further with no response :-("); return err;},
    };
    ...
    let body = match response_lo.read_to_end() {
     ...
     let ret =  str::from_utf8(body.clone().as_slice()).expect( return err ;).clone().to_string();
     return ret;
